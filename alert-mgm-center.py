...
# Initialize robust mock client alerts to perfectly populate the visualizations
...
def calculate_risk_index(row):
    ...
    elif row['Severity'] == "Event" or row['Severity'] == "Status": score += 15
...
# ==========================================
    ...
    if st.session_state.client_portal_view == "list":
        ...
        # 1. Base Data Loading and Risk Scoring
        df = st.session_state.client_alerts_db.copy()
        
        # Compute Dynamic Risk Index (1-100)
        ...
        with cat_tabs[0]: # Dashboard Overview
            # =========================================================
            # ZONE 1: ACT NOW (Top Triage Banner)
            # =========================================================
            st.markdown("### 🚨 Act Now: Critical Priorities")
            st.write("Top issues sorted by proprietary Risk Index (Severity + Impact + Unresolved Time).")
            ...
            if top_alerts.empty:
                ...
                for idx, (_, alert) in enumerate(top_alerts.iterrows()):
                    with cols[idx]:
                        # Determine card styling based on Risk Score
                        ...
                        # Action Buttons
                        b1, b2 = st.columns(2)
                        if b1.button("Assing to Manager", key=f"assign_{alert['Alert ID']}", use_container_width=True):
            ...
            # =========================================================
            # ZONE 2 & 3: IMPACT HEATMAP & PROACTIVE WATCHLIST
            # =========================================================
            ...
            with col_heatmap:
                ...
                # Group risk by supplier
                if not df.empty and MATPLOTLIB_AVAILABLE:
                    sup_risk = df.groupby('Supplier')['Risk Score'].sum().sort_values(ascending=True)
                    
                    fig, ax = plt.subplots(figsize=(6, 4))
                    # Color map based on risk sum severity
                    colors = ['#ef4444' if score > 300 else '#f59e0b' if score > 150 else '#3b82f6' for score in sup_risk]
                    bars = ax.barh(sup_risk.index, sup_risk.values, color=colors, height=0.6)
                    
                    for bar in bars:
                        width = bar.get_width()
                        ax.text(width + 5, bar.get_y() + bar.get_height()/2, f"Risk: {int(width)}", ha='left', va='center', fontsize=9, fontweight='bold', color='#475569')
            ...
            with col_watchlist:
                ...
                watchlist_df = df[
                    (df['Severity'] == 'Level 2') | 
                    (df['Alert Type'].str.contains('EOL'))
                ].sort_values(by='Date/Time', ascending=False).head(5)
            ...
            # =========================================================
            # ZONE 4: SMART TRIAGE WORKFLOW FEED
            # =========================================================
            st.markdown("### 📋 Smart Triage Queue")
            st.write("Complete ledger of all active alerts, strictly sorted by the Priority Risk Index.")
            
            if not df.empty:
                # Format metrics for clean table display
                triage_df = df.copy()
                triage_df['Trigger Info'] = triage_df.apply(format_trigger_value, axis=1)
                
                # Select workflow columns
                workflow_cols = ['Risk Score', 'Alert ID', 'Alert Name', 'Tool', 'Supplier', 'Plant', 'Status', 'Owner']
                triage_display = triage_df[workflow_cols].copy()
                
                # Interactive Data Editor for workflow status
                st.data_editor(
                    triage_display,
                    column_config={
                        "Risk Score": st.column_config.ProgressColumn(
                            "Priority (1-100)", help="Calculated Risk Index",
                            format="%d", min_value=0, max_value=100,
                        ),
                        "Status": st.column_config.SelectboxColumn(
                            "Workflow Status",
                            help="Update the current resolution status.",
                            options=["Open", "Investigating", "Parts Ordered", "Resolved"],
                            required=True
                        ),
